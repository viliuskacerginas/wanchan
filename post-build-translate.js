...
const TARGET_DIR = path.join(__dirname, 'builds'); // user said build/, but their script says builds/ - let's check builds/ first since I saw it in package.json
if (!fs.existsSync(TARGET_DIR)) {
    console.log("No builds/ directory found. Checking build/ just in case...");
    const fallbackDir = path.join(__dirname, 'build');
    if (fs.existsSync(fallbackDir)) {
        process.chdir(fallbackDir);
    } else {
        console.error("Could not find build or builds directory.");
        process.exit(0);
    }
} else {
...
function translateNode(node) {
    if (node.type === 'text') {
        ...
        if (TRANSLATIONS[text]) {
            node.data = TRANSLATIONS[text];
        } else if (TRANSLATIONS[normalized]) {
            node.data = TRANSLATIONS[normalized];
        } else {
            node.data = text.replace(/Yomitan/g, 'Wanchan');
        }
    }
}

function translateHTML(htmlContent) {
    ...
    const attrsToTranslate = ['placeholder', 'title', 'alt', 'aria-label'];
    ...
    $('*').each(function() {
        ...
        attrsToTranslate.forEach(attr => {
            ...
            if (val) {
                ...
                if (TRANSLATIONS[val]) {
                    el.attr(attr, TRANSLATIONS[val]);
                } else if (TRANSLATIONS[normalized]) {
                    el.attr(attr, TRANSLATIONS[normalized]);
                } else {
                    el.attr(attr, val.replace(/Yomitan/g, 'Wanchan'));
                }
            }
        });
        ...
        if (el.is('input')) {
            ...
            if (type === 'button' || type === 'submit' || type === 'reset') {
                ...
                if (val) {
                    const normalized = normalizeText(val);
                    if (TRANSLATIONS[val]) {
                        el.attr('value', TRANSLATIONS[val]);
                    } else if (TRANSLATIONS[normalized]) {
                        el.attr('value', TRANSLATIONS[normalized]);
                    } else {
                        el.attr('value', val.replace(/Yomitan/g, 'Wanchan'));
                    }
                }
            }
        }
...
zipFiles.forEach(zipFile => {
    ...
    zipEntries.forEach(entry => {
        if (!entry.isDirectory && entry.entryName.endsWith('.html')) {
            ...
            const newContent = translateHTML(content);
    ...
    if (modified) {
        console.log(`Saving modifications to ${zipFile}...`);
        zip.writeZip(zipFile);
    }
});
console.log('Post-build translation complete.');
